...
const MenuItem: React.FC<MenuItemProps> = ({
  ...
  const marqueeText = description || text;
  ...
  useEffect(() => {
    ...
    const setupMarquee = () => {
      ...
      if (!marqueeContent) return;
      const contentWidth = marqueeContent.offsetWidth;
      // layout/fonts/image may not be ready on first paint — retry instead of
      // bailing for good (which left the band static and half-empty).
      if (contentWidth === 0) {
        if (retries++ < 30) setTimeout(setupMarquee, 100);
        return;
      }
