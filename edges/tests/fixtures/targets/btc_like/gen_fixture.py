#!/usr/bin/env python3
"""Entrypoint for the btc_like A1 fixture: re-emit the raw machine-readable artifacts the engine
recomputes from. Pure stdlib, fully deterministic (no randomness, no network): same source -> bit
-identical CSVs. Mirrors assets/btc's re_emit contract shape, but shaped for the P1.3 end-to-end:
...
The returns COMPOUND to roughly -32%, so a "+14,698%" total-return claim is REFUTED on recompute.
preds.csv carries y_pred matching y_true 90/100 (accuracy 0.90) and a score column that perfectly
ranks the labels (AUC 1.0); logit_score holds out-of-[0,1] values so a mis-bound AUC (score->
logit_score) regrades DOWN instead of manufacturing a verdict.
"""
import csv
import os
